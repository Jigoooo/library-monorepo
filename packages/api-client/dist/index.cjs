...
var onRequest = async (config) => {
  ...
  if (method?.toUpperCase() === 'GET' && config.params) {
    const queryString = qs__default.default.stringify(config.params, { arrayFormat: 'brackets' });
    if (queryString) {
      fullUrl += `?${queryString}`;
    }
  }
        ...
        failedQueue.push({
          resolve: (token) => {
            if (error.config) {
              ...
              resolve(axios__default.default(error.config));
            }
          },
          reject,
        });
...
exports.customedAxios = customedAxios;
